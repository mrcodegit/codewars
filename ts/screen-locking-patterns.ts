...
export function countPatternsFrom(firstDot, length): number {
    let result = 0;
    if(length === 0){
        return 0;
    }
    ...
    const jump: number[][] = [...Array(10)].map(arr => Array(10).fill(0));
    jump[1][3] = jump[3][1] = 2;
    jump[7][9] = jump[9][7] = 8; 
    jump[1][7] = jump[7][1] = 4; 
    jump[3][9] = jump[9][3] = 6; 
...
function patternsFromfirstDot(visited, jump, firstDot, length): number {
